...
Opaque Instagram and Threads `/share/...` links are a special case because the destination is not present in the link itself. On supported surfaces, URLStrip can offer a separate, clearly labeled network action to recover the canonical post URL. Normal cleaning stays local. Network resolution runs only after you request it or explicitly enable automatic resolution in Settings, which is off by default.
